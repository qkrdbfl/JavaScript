...
//3) + : 앞선 패턴이 최소 한 번 이상 반복되는 문자열 (반복 검색)
//   +는 {1,}과 같다
// b가 최소 1번 이상 반복된것
target = 'a aa aaa b bb bbb ab aab abb';
...
//4) ? : 앞선 패턴이 최대 한 번(0번 포함) 이상 반복되는 문자열 (반복 검색)
//   ?는 {0,1}과 같다.
target = 'soul seoul';
// s 다음 e가 최대 한번(0번 포함) 이상 반복 되고 oul이 이어지는 문자열 전역 검색
console.log(target.match(/se?oul/g)); // [ 'soul', 'seoul' ] //e?는 옵셔널한 부분임.
console.log('--------------------------------------------');
...
// [] 내의 문자는 or로 동작
console.log(target.match(/[abc]+/g)); // [ 'aa', 'bb' ]
...
console.log(target.match(/\d+/g)); // [ '123', '456' ]
console.log(target.match(/\D+/g)); // [ 'aa bb cc dd ', ' ', ' _@' ]
console.log(target.match(/\w+/g)); //밑에 출력 적음(1)
console.log(target.match(/\W+/g)); //밑에 출력 적음(2)
console.log('-------------------------------------------');
/* (1)
    [
    'aa', 'bb',
...
console.log(/^https/.test(target)); // true // https로 시작하는지 검사
console.log(/com$/.test(target)); // true // com으로 끝나는지 검사
