...
//chickenList.unshift('간장치킨', '마늘치킨'); //따로 넣는거랑 한꺼번에 넣는거랑 순서 다름
...
// slice(시작인덱스, 종료인덱스)
console.log(`front.slice(1, 3) : ${front.slice(1, 3)}`);   //slice: 잘라서 반환. 1,2만 잘라서 반환함                     // CSS,JavaScript
console.log(`front : ${front}`);                                                // HTML,CSS,JavaScript,jQuery

// splice(index, 제거수, 추가값1, 추가값2, ...)
console.log(`front.splice(3, 1, "React") : ${front.splice(3, 1, "React")}`);    // jQuery //제거된 값 반환됨
console.log(`front : ${front}`);                                                // HTML,CSS,JavaScript,React //원본 배열 변경됨

// Array.prototype.join : 배열을 구분자로 결합하여 문자열로 반환
const snackList = ['사탕', '초콜렛', '껌', '과자'];
console.log(`snackList.join() : ${snackList.join()}`);          // 사탕,초콜렛,껌,과자 ( , 가 기본)
console.log(`snackList.join('/') : ${snackList.join('/')}`);    // 사탕/초콜렛/껌/과자

// Array.prototype.reverse : : 배열의 순서를 뒤집음(역순서)
